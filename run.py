...
class DMM(nn.Module):
    ...
    def _model(
        ...
        seq_mask = (
            seq_mask if seq_mask is not None else torch.ones(z0.size(0), time_steps)
        )
        ...
        with pyro.plate("data", batch_dim):
            for t in pyro.markov(range(time_steps)):
                ...
                with poutine.scale(None, annealing):
                    z = pyro.sample(
                        f"z_{t+1}", Normal(z_loc, z_scale).mask(m).to_event(1)
                    )
...
def pack_sequences(
    batch: List[Tensor],
) -> Tuple[Tensor, PackedSequence, Tensor, Tensor]:
